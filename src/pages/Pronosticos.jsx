...
export default function Pronosticos({ participante }) {
  ...
  async function fetchData() {
    ...
    const { data: jugs1 } = await supabase
      .from('jugadores').select('nombre, numero, equipo, api_player_id').range(0, 999)
    const { data: jugs2 } = await supabase
      .from('jugadores').select('nombre, numero, equipo, api_player_id').range(1000, 1999)
    const jugs = [...(jugs1 || []), ...(jugs2 || [])]

    const pronosMap = {}
    const enviadosMap = {}
    ;(pronos || []).forEach(pr => {
      pronosMap[pr.partido_id] = { local: pr.goles_local, visita: pr.goles_visita, scorer: pr.primer_anotador, scorerId: pr.anotador_id }
    ...
    ;(jugs || []).forEach(j => {
      ...
      jugsMap[j.equipo].push({ nombre: j.nombre, numero: j.numero, id: j.api_player_id })
    })
  ...
  function updateProno(partidoId, field, value) {
    ...
    setPronosticos(prev => ({
      ...prev,
      [partidoId]: { ...(prev[partidoId] || { local: 0, visita: 0, scorer: '', scorerId: null }), [field]: value }
    }))
  }

  function updateScorer(partidoId, optionValue) {
    if (enviados[partidoId]) return
    let scorer = ''
    let scorerId = null
    if (optionValue === 'autogol') {
      scorer = 'autogol'
    } else if (optionValue) {
      const sep = optionValue.indexOf('|')
      scorerId = sep > 0 ? parseInt(optionValue.slice(0, sep)) : null
      scorer = sep > 0 ? optionValue.slice(sep + 1) : optionValue
    }
    setPronosticos(prev => ({
      ...prev,
      [partidoId]: { ...(prev[partidoId] || { local: 0, visita: 0, scorer: '', scorerId: null }), scorer, scorerId }
    }))
  }
              ...
              <div style={s.scorerRow}>
                ...
                {bloqueado ? (
                  ...
                  <select style={s.scorerSelect}
                    value={pr.scorerId ? pr.scorerId + '|' + pr.scorer : (pr.scorer === 'autogol' ? 'autogol' : '')}
                    onChange={e => updateScorer(p.id, e.target.value)}>
                    <option value="">— Selecciona jugador —</option>
                    <option value="autogol">Autogol</option>
                    <optgroup label={p.equipo_local}>
                      {jugLocal.map(j => <option key={j.id || j.nombre} value={(j.id || '') + '|' + j.nombre}>{j.numero ? j.numero + ' · ' + j.nombre : j.nombre}</option>)}
                    </optgroup>
                    <optgroup label={p.equipo_visita}>
                      {jugVisita.map(j => <option key={j.id || j.nombre} value={(j.id || '') + '|' + j.nombre}>{j.numero ? j.numero + ' · ' + j.nombre : j.nombre}</option>)}
                    </optgroup>
                  </select>
                ) : (
